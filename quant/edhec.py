...
def get_ffme_returns():
    ...
    """
    me_m = pd.read_csv(
        f"{DATA_DIR}/Portfolios_Formed_on_ME_monthly_EW.csv",
        header=0,
        index_col=0,
        na_values=-99.99,
    )
...
def get_hfi_returns():
    ...
    """
    hfi = pd.read_csv(
        f"{DATA_DIR}/edhec-hedgefundindices.csv",
        header=0,
        index_col=0,
        parse_dates=True,
    )
...
def get_ind_file(filetype):
    ...
    if filetype == "returns":
        ...
        name = "size"
        divisor = 1
    ind = pd.read_csv(f"{DATA_DIR}/ind30_m_{name}.csv", header=0, index_col=0) / divisor
...
def bt_mix(r1, r2, allocator, **kwargs):
    ...
    if not weights.shape == r1.shape:
        raise ValueError(
            "Allocator returned weights with a different shape than the returns"
        )
